	...
	if (!version.os_family eq 'unix') then begin        ; auf der Sun
		...
		cd,cddir(1)		
              ...
              ;  cd,lw_path
                ...
                complete_file='/pf/u/u241112/'+namevar(laufvar) ; Hier wurde die Variable eingetragen !!! 
                print, complete_file 
                print, lw_path
            endelse
  ...
  if (!version.os_family eq 'unix') then begin        ; auf der Sun
    ...
    cd,cddir(1)   
              ...
              ;  cd,lw_path
                cd, datenort
                ; cd, '/pf/u/u241112/'
                ;cd, '/users/ifmsun27b/ifmrs/u241068/pool/satellites/ERS-1/'
                ;cd, '/users/ifmsun27b/ifmrs/u241068/pool/satellites/ERS-2/'
                ;complete_file=DIALOG_PICKFILE(/READ, FILTER = '*.sar')
                ;spawn,'ls',namevariable
                print, namevar(laufvar)
                ;print, sarname
                complete_file=datenort+namevar(laufvar)
                ;complete_file='/pf/u/u241112/'+namevar(laufvar)
                ;complete_file='/users/ifmsun27b/ifmrs/u241068/pool/satellites/ERS-1/'+namevar(laufvar) ; Hier wurde die Variable eingetragen !!! 
                ;complete_file='/users/ifmsun27b/ifmrs/u241068/pool/satellites/ERS-2/'+namevar(laufvar) ; Hier wurde die Variable eingetragen !!! 
                print, complete_file 
                print, datenort
                ;print, lw_path
            endelse
...
print,'habe die datei gefunden und sie wird jetzt eingelesen.'
...
;          exabyte=ecd+file
          ...
          if (ecd ne 'QL') then begin            ; QL nicht in Datenbank
              ...
              tape_form='users/ifmsun27b/ifmrs/u241068/pool/satellites/ERS-1/'+tape_form
              print, tape_form
